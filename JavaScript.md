...
자바스크립트에서 비동기 처리를 하는 방법에는 콜백 함수와 프로미스가 있다. 

*콜백 함수 (함수의 매개변수를 통해서 다른 함수 내부로 전달되는 함수를 콜백함수라고 한다  즉, 매개변수로 함수를 전달하여 실행한다)

하지만 콜백 함수를 사용할 경우 콜백 헬이 발생할 수 있다 

#### 콜백 패턴의 한계 

1. 콜백 헬 
비동기 함수의 경우 비동기 처리 결과를 외부에 반환할 수 없고 상위 스코프의 변수에 할당 할 수 없다 그러므로 비동기 함수의 처리 결과에 대한 후속 처리는 비동기 함수 내에서 이루어져아한다 
이때 후속 처리를 하기 위해서 콜백함수를 사용하는데 비동기 처리가 성공했을 때의 호출될 콜백 함수, 실패했을 때의 콜백 함수가 전달 될 수 있다 
이처럼 비동기 함수가 비동기 함수 결과를 처리하기 위해서 비동기 함수를 또 다시 호출 해야 한다면 콜백 함수 호출이 계속 해서 중첩되어 복잡도가 높아지는 현상이 발생한다. 이를 콜백 헬이라 부른다. 


2. 에러 처리의 한계 
비동기 함수 중 setTimeout함수를 예로 들어 에러 처리의 한계를 살펴보자면, setTimeout이 호출되면 콜 스택에 푸시되어 실행되기 된다 이때 setTimeout함수는 비동기 함수이기 때문에 콜백 함수가 실행되는 것을 기다리지 않고 즉시 종료되어 콜 스택에서 제거된다 
하지만 에러는 호출자의 방향으로 전파된다 즉 콜 스택의 아래방향으로 전파되는데 setTimeout은 이미 콜 스택에서 제거되었기 때문에 setTimeout의 콜백함수가 발생 시킨 에러는 catch되지 않는다. 
